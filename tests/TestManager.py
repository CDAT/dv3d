...
class TestManager:
    ...
    def runTest(self, testName, interactive=False, baselinedir=None ):
        ...
        if test is None:
            ...
            print(" Defined Tests = %s " % str( TestManager.DefinedTests.items() ), file=sys.stderr)
            return -1
...
#        test_image = os.path.join( self.test_dir, 'images', '.'.join( [ self.name, 'png' ] ) )
        ...
        return test_image, self.image_name
